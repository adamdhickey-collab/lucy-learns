...
**AC-1.4** [ ] A household that picks *Start empty* is never shown a number
...
**AC-2.1** [ ] A handler can complete a full session one-handed, with a leash
...
**AC-2.4** [ ] During the repetition count, the handler can see what a
...
**AC-2.5** [ ] "Lucy is too excited" is reachable at any point in a session
...
**AC-2.7** [ ] A session can be abandoned midway without logging anything.
...
**AC-3.3** [ ] A handler can override the app's choice of level at any time,
...
**AC-3.7** [ ] Mastery never claims reliability from a single day.
*Observe:* three good sessions on one day do not produce "Reliable".

## 4. What the trainer reads

**AC-4.1** [ ] Every number the trainer sees is derived from a session the
...
**AC-5.2** [ ] The app never claims a training outcome the trainer has not
...
**AC-6.7** [ ] No console errors during a complete session.
